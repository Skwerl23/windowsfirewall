...
use windows::Win32::NetworkManagement::WindowsFirewall::{
    INetFwPolicy2, INetFwRule, NetFwPolicy2, NetFwRule, NET_FW_ACTION_BLOCK, NET_FW_IP_PROTOCOL_TCP,
};
...
fn add_firewall_rule(rule_name: String) {
    unsafe {
        ...
        let rule: INetFwRule = CoCreateInstance(&NetFwRule, None, CLSCTX_INPROC_SERVER).unwrap();
        ...
        let _ = rule.SetDescription(&BSTR::from(
            "This rule was created by rust. It blocks a list of IPs.",
        ));
    ...
    unsafe {
        ...
        if let Ok(rules) = fw_policy.as_mut().unwrap().Rules() {
            rules
                ...
                .expect("Couldn't add rule"); // add new rule
